...
def run_demo() -> int:
    ...
    code |= _run([sys.executable, "tools/validate_runbooks.py", "--mode", "demo"], cwd=REPO_ROOT)
...
def run_production() -> int:
    ...
    env = dict(os.environ)
    env.setdefault("COMPOSE_PROJECT_NAME", "repo19_prodtests")
    ...
    try:
        ...
        code |= _run([sys.executable, "tools/validate_runbooks.py", "--mode", "production"], cwd=REPO_ROOT, env={**env, "PRODUCTION_RUNBOOKS_CONFIRM": "1"})
        return code
    finally:
